...
User Function MT097END()
	...
	Do while .not. eof()
	   If C7_FILIAL + C7_NUM <> xFilial("SC7") + cPedido
	      Exit
	   Endif
...
Static Function ApNivel(aPed)
	...
	cBody += ImpCab(aPed[1][2], cFornece)
	For xY :=1 To nItens
		aItem := {'','','','','','',''}
		aItem[1] := aPed[xY,3]
...
Static Function ImpCab(cPed, cFornece)
	Local cRet := ''
	cRet += '	<tr>'
